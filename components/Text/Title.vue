<template>
    <div class="flex flex-col items-center justify-center space-y-2 sm:space-y-4 text-center">
      <!-- Title with Button -->
      <span class="flex items-center justify-center space-x-2 sm:space-x-4">
        <h1 
          :class="props.additionalClass" 
          class="text-3xl sm:text-5xl cute-font font-bold leading-tight">
          {{ props.text }}
        </h1>
        <button 
      ...
      <!-- Subtext -->
      <p class="text-lg sm:text-base text-slate-50 cute-font pt-5">{{ props.subtext }}</p>
    </div>
  </template>
